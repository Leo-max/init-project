        ...
        <el-table v-if="tableShowFlag"
          ...
          <el-table-column fixed="right" label="操作" width="100" >
            <template slot-scope="scope">
              <a @click="handleClick(scope.row, 1)" type="text" size="small">通过</a>
              <a @click="handleClick(scope.row, 2)" type="text" size="small">拒绝</a>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    handleClick (item, type) {
      this.$api.submitSend({id: item.id, status: type}).then(
        res => {
          if (res.code == 0) {
            success('操作成功')
            this.renderData()
    ...
    renderData () {
      ...
      this.$api.getTixianList(data).then(
        res => {
          this.loading = false
          if (res.code == 0) {
            ...
            this.orderList.forEach(
              item => {
                item.ctimeStr = util.formatDate(item.createdAt+'000')
                if(item.status === 0){
                  item.status = '未审核'
                }
                if(item.status === 1){
