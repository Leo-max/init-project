        ...
        <el-table v-if="tableShowFlag"
          ...
          <el-table-column fixed="right" label="操作" width="100" >
            <template slot-scope="scope">
              <a v-if="scope.row.status == 0" @click="handleClick(scope.row, 1)" type="text" size="small">上架</a>
              <a v-if="scope.row.status == 1" @click="handleClick(scope.row, 0)" type="text" size="small">下架</a>
            </template>
          </el-table-column>
          <!-- <el-table-column fixed="right" label="操作" width="100" >
            <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    handleClick (item, status) {
      this.$api.upDown({id: item.id, status: status}).then(
        res => {
          if (res.code == 0) {
            success('操作成功')
            this.renderData()
    ...
    limitUser (user) {
      this.user = user
      this.userRole = ''
      this.$api.getUserRole({userId: user.id})
        .then(
          res => {
            if (res.ack) {
              if (res.returnContent && res.returnContent.length) {
                this.userRole = res.returnContent[0]
    ...
    renderData () {
      ...
      this.$api.getGoodList(data).then(
        res => {
          this.loading = false
          if (res.code == 0) {
            ...
            this.orderList.forEach(
              item => {
                item.statusStr = item.status == 1 ? '上架中' : '已下架'
              }
            )
    ...
    getRoleList () {
      let data = Object.assign({}, this.$pageData)
      data.pageNum = this.roleCurrentPage
      this.$api.getRoleList(data)
        .then(
          res => {
            if (res.ack) {
              this.roleList = res.returnContent.result
              this.roleTotal = res.returnContent.totalCount
            }
          }
        )
    }
